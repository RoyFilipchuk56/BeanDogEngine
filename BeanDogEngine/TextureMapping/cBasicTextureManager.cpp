...
bool cBasicTextureManager::Create2DTextureFromBMPFile( std::string textureFileName, bool generateMIPMap )
{
	std::string fileToLoadFullPath = this->m_basePath + "/" + textureFileName;


	CTextureFromBMP* pTempTexture = new CTextureFromBMP();
	if ( ! pTempTexture->CreateNewTextureFromBMPFile2( textureFileName, fileToLoadFullPath, generateMIPMap) )
	{
...
void cBasicTextureManager::GetLoadedTextureList(std::vector<std::string>& vecTexturesLoaded)
{
	// Copy the map contents (texture name) into the vector:
		...
		itTexture++)
	{
		...
		std::string textureName = itTexture->second->getTextureName();
		vecTexturesLoaded.push_back(textureName);
	}
	return;
}
